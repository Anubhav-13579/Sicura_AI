...
`;
function parseFlag(args, name, hasValue = true) {
...
async function run(argv) {
    ...
    switch (command) {
        case 'login': {
            ...
            const org = parseFlag(rest, 'org');
            const backend = parseFlag(rest, 'backend');
            const product = parseFlag(rest, 'product');
        ...
        case 'switch-org': {
            ...
            const keyName = parseFlag(rest, 'key-name');
            const target = rest.find((a) => !a.startsWith('--'));
            if (!target)
                throw new util_1.CLIError('Usage: armoriq switch-org <name-or-id>');
        ...
        case 'keys': {
            ...
            if (sub === 'revoke') {
                const id = rest[1];
                if (!id)
                    throw new util_1.CLIError('Usage: armoriq keys revoke <id>');
            ...
            (0, util_1.out)('Usage: armoriq keys list | revoke <id> | prune [--yes]');
            return 1;
        }
        case 'status': {
        ...
        case 'validate':
        case 'register': {
            (0, util_1.out)(`'armoriq ${command}' is not yet implemented in the TypeScript CLI.\n` +
        ...
        default:
            (0, util_1.out)(`Unknown command: ${command}`);
            (0, util_1.out)(HELP);
            return 1;
    }
}
if (require.main === module) {
    ...
    run(argv)
        ...
        .catch((err) => {
        if (err instanceof util_1.CLIError) {
            (0, util_1.out)(`  ${util_1.CROSS} ${err.message}`);
            process.exit(1);
        }
        (0, util_1.out)(`  ${util_1.CROSS} Unexpected error: ${err.message}`);
        process.exit(1);
    });
}
